    ...
    async.waterfall([
        ...
        function(balances, callback){
            ...
            for (var worker in payments){
                ...
                transferCommands[commandIndex].rpc.destinations.push({amount: (amount/config.coinUnits), address: worker});
            ...
            async.filter(transferCommands, function(transferCmd, cback){
                apiInterfaces.rpcWallet(config.wallet.rpcMethod, transferCmd.rpc, function (error, result) {
                    if (error){
