    ...
    async.parallel({
        pool: function(callback){
            redisClient.multi(redisCommands).exec(function(error, replies){
                ...
                for (var i = 0; i < hashrates.length; i++){
                    ...
                    log('info', logSystem, hashParts[0] + ' ' + hashParts[1] + ' ' + hashParts[2]);
                }
                ...
                for (var miner in minersHashrate){
                    ...
                    workerStats.push({ workerAddress: maskAddress(miner), workerHashRate: Math.round((shares / config.api.hashrateWindow) * 100) / 100 });
                }
        ...
        config: function(callback){
            callback(null, {
                ...
                paymentInterval: config.payments.interval,
...
function maskAddress(address) {
  ...
  return address.substr(address, address.length - 20) + '********************';
}
...
function handleGetWorkerStats(response) {
  var reply = JSON.stringify(workerStats);
  response.writeHead("200", {
    'Access-Control-Allow-Origin': '*',
    'Cache-Control': 'no-cache',
    'Content-Type': 'application/json',
